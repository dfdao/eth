...
async function subgraphCodegen(_args: HardhatArguments, hre: HardhatRuntimeEnvironment) {
  ...
  const startBlock = isDev ? (await hre.ethers.provider.getBlockNumber()).toString() : START_BLOCK.toString();
  ...
  const yaml = (await fs.readFile(path.join(subgraphPath, 'subgraph.template.yaml')))
    ...
    .replace(/{{{DARKFOREST_ABI_PATH}}}/g, path.join(abisPath, 'DarkForest_stripped.json')); /* doing one more time for Subgraph template */
