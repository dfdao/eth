...
describe('Arena Functions', function () {
  ...
  describe('Manual Spawn', async function () {
    ...
    it('reverts if planet not initialized as a spawn planet', async function () {
      await expect(
        world.user1Core.arenaInitializePlayer(...makeInitArgs(ADMIN_PLANET),1)
    ...
    it('reverts if spawn planet already initialized', async function () {
      ...
      const toPlanetExtended = await world.contract.planetsExtendedInfo(ADMIN_PLANET.id);
      expect(toPlanetExtended.isInitialized).to.equal(true);

      await expect(world.user1Core.arenaInitializePlayer(...makeInitArgs(ADMIN_PLANET),1))
        .to.emit(world.contract, 'PlayerInitialized')
        .withArgs(world.user1.address, ADMIN_PLANET.id.toString());

      await expect(
        world.user2Core.arenaInitializePlayer(...makeInitArgs(ADMIN_PLANET),1)
    ...
    it('allows player to spawn at admin planet that is initialized', async function () {
      ...
      await expect(world.user1Core.arenaInitializePlayer(...makeInitArgs(ADMIN_PLANET_CLOAKED),1))
        .to.emit(world.contract, 'PlayerInitialized')
    ...
    it('sets the planet to the proper values', async function () {
      ...
      await world.user1Core.arenaInitializePlayer(...makeInitArgs(LVL2_PLANET_DEEP_SPACE),1);
  ...
  describe('Threshold for Planet Location Id validity', function () {
    ...
    it('Planet that has id below difficulty but above L0 threshold is not valid', async function () {
      ...
      await expect(
        world.user1Core.arenaInitializePlayer(...makeInitArgs(SPAWN_PLANET_1),1)
  ...
  describe('Teams', function() {
    ...
    it('does not allow players to join an invalid team', async function () {
      await expect(world.user1Core.arenaInitializePlayer(...makeInitArgs(SPAWN_PLANET_1), 5)).to.be.revertedWith('invalid team');
    });

    it('allows players to join a valid team', async function () {
      await world.user1Core.arenaInitializePlayer(...makeInitArgs(SPAWN_PLANET_1), 1);
