...
import { ADMIN_PLANET_CLOAKED, SPAWN_PLANET_1, SPAWN_PLANET_2 } from './utils/WorldConstants';
...
describe('Claim Victory', function () {
  ...
  describe('invading target planet', function () {
    it('player cannot invade target planet without ownership', async function () {
      await expect(
        world.user1Core.invadeTargetPlanet(...makeRevealArgs(ADMIN_PLANET_CLOAKED, 10, 20))
    ...
    describe('player owns target planet', async function () {
      beforeEach(async function () {
        ...
        await world.user1Core.move(
          ...makeMoveArgs(SPAWN_PLANET_1, ADMIN_PLANET_CLOAKED, dist, shipsSent, silverSent)
        );
      });
      it('player can invade target planet', async function () {
        await expect(world.user1Core.invadeTargetPlanet(...makeRevealArgs(ADMIN_PLANET_CLOAKED, 10, 20)))
          .to.emit(world.contract, 'TargetPlanetInvaded')
          .withArgs(world.user1.address, ADMIN_PLANET_CLOAKED.id);
      });
    });
  });
  ...
  describe('claiming victory on target planet', function () {
    beforeEach(async function () {
      ...
      await world.user1Core.invadeTargetPlanet(...makeRevealArgs(ADMIN_PLANET_CLOAKED, 10, 20));
    });
    it('needs to hold planet for enough time', async function () {
      await expect(world.user1Core.claimTargetPlanetVictory(ADMIN_PLANET_CLOAKED.id)).to.be.revertedWith(
    ...
    describe('time elapsed', async function () {
      ...
      it('must own planet to claim victory', async function () {
        await expect(world.user2Core.claimTargetPlanetVictory(ADMIN_PLANET_CLOAKED.id)).to.be.revertedWith(
          'you can only claim victory with planets you own'
        );
      });
      it('should emit event', async function () {
       
        const planet = await world.contract.planetsExtendedInfo2(ADMIN_PLANET_CLOAKED.id);
        await expect(world.user1Core.claimTargetPlanetVictory(ADMIN_PLANET_CLOAKED.id))
      ...
      it('sets gameover to true and winner to msg sender', async function () {
        await world.user1Core.claimTargetPlanetVictory(ADMIN_PLANET_CLOAKED.id);
